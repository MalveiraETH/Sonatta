...
Deno.serve(async (req) => {
  try {
    ...
    for (const sale of allSales) {
      ...
      let hasVisaMaster = details.some(d => isVisaMaster(d.card_brand));
      if (!hasVisaMaster) continue;
      ...
      dryRun.push({
        ...
        old_fee: sale.total_fee_amount,
        new_fee: totalFeeAmount,
        old_net: sale.total_net_amount,
        new_net: totalNetAmount,
      });
